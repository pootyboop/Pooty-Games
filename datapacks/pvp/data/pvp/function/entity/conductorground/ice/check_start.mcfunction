...
execute as @n[type=#pvp:fighter,tag=fighter,distance=..3] if items entity @s armor.head *[minecraft:custom_data~{component:"permafrost"}] as @n[type=marker,tag=currconductor,tag=!c_ice] at @s run return run function pvp:entity/conductorground/ice/start with storage pvp:storage temp