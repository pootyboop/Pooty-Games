execute as @p[gamemode=!spectator] run function bedwars:ore_ct

execute as @p[gamemode=!spectator,distance=..6,scores={ironCt=10..}] at @s run function bitem:stone_sword_2
execute unless entity @p[gamemode=!spectator,distance=..6,scores={ironCt=10..}] run function bitem:reject


clear @p[gamemode=!spectator] stone_sword[custom_data~{bmenu1:1b}]
function bmenu1:weapons_setup