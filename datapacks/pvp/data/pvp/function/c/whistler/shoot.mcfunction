...
scoreboard players operation dummy dummy = @s whistler.chargelevel
execute store result storage pvp:storage temp.whistler.damage double 0.5 run scoreboard players get dummy dummy

function pvp:c/whistler/shoot2 with storage pvp:storage temp.whistler
execute as @e[type=arrow,tag=whistler,distance=..3] at @s run function pvp:c/whistler/shoot_arrow_special with storage pvp:storage temp.whistler
