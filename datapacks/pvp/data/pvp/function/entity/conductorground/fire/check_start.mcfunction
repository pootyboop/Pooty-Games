...
execute as @n[predicate=pvp:fighter,distance=..3] if items entity @s armor.head *[minecraft:custom_data~{component:"hothead"}] as @n[type=marker,tag=currconductor] at @s run return run function pvp:entity/conductorground/fire/start with storage pvp:storage temp
execute as @n[predicate=pvp:fighter,distance=0.1..3,predicate=pred:fire] as @n[type=marker,tag=currconductor] at @s run return run function pvp:entity/conductorground/fire/start with storage pvp:storage temp