...
$execute as @e[distance=..$(radius),type=marker,tag=mine,tag=!explodingmine] at @s run function pvp:e/mine/explode
$execute as @e[distance=..$(radius),type=slime,tag=trap] run damage @s $(dmg) explosion by @n[predicate=pvp:fighter,tag=explosionowner] from @n[predicate=pvp:fighter,tag=explosionowner]
execute as @e[distance=..4.5,type=marker,tag=conductorground,tag=!currexplodingconductor] at @s run function pvp:e/conductorground/explosion/start
