...
tag @a[tag=!leaped,tag=midair,predicate=pred:grounded] remove midair
...
execute if score dummy minigametimer matches 0 unless entity @e[type=pig,tag=ball,predicate=pred:grounded] run title @a title {"text":""}
execute if score dummy minigametimer matches 0 unless entity @e[type=pig,tag=ball,predicate=pred:grounded] run title @a subtitle {"text":"FINAL SHOT!"}
execute if score dummy minigametimer matches 0 if entity @e[type=pig,tag=ball,predicate=pred:grounded] run function hamsoccer:endgame
