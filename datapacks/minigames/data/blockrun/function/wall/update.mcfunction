...
execute as @a[gamemode=!spectator] at @s run function blockrun:player/wall_check
...
function blockrun:wall/sched with storage minigames:temp blockrun

# new wall
scoreboard players remove dummy blockrun.currdist 1
execute if score dummy blockrun.currdist matches ..0 run return run function blockrun:wall/pick/1
