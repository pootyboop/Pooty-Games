#execute unless entity @n[type=trident,distance=.1..] run schedule clear pvp:c/ranged/trident/sched
...
#execute at @a if score @s dummy = @p uuid0 as @p run function pvp:c/ranged/trident/return2 with storage pvp:storage temp
#kill @s