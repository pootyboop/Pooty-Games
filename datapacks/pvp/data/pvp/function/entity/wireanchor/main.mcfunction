execute at @n[type=slime,tag=wireanchor,distance=..2,predicate=pvp:hurt/just_hurt] if score @s uuid0 = @n[type=slime,tag=wireanchor] uuid0 at @s run return run function pvp:entity/kill {"entity":"wireanchor"}
