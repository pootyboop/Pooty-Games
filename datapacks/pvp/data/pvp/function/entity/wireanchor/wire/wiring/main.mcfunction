execute positioned ~ ~-1 ~ at @n[type=#pvp:fighter,tag=fighter,distance=..1] unless score @s uuid0 = @n[type=#pvp:fighter,tag=fighter] uuid0 run return run function pvp:entity/wireanchor/wire/wiring/connected
...
#execute positioned ~ ~-1.35 ~ if entity @n[type=#pvp:fighter,tag=fighter,distance=..1,tag=currwiretarget] run return fail
