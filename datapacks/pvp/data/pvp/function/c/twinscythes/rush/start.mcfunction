...
execute facing entity @n[type=marker,tag=rushtarget,tag=currrushtpmarker] feet run function pvp:c/twinscythes/rush/particle_trail

execute as @n[type=marker,tag=rushtarget,tag=currrushtpmarker] at @s facing entity @n[predicate=pvp:fighter,tag=startrush] feet rotated ~180 ~ run function pvp:c/twinscythes/rush/start_marker

execute if entity @s[tag=maxedrush] run function pvp:c/salvo/explode
execute if entity @s[tag=maxedrush] run execute at @s run function pvp:c/salvo/explode

scoreboard players set @s twinscythes.timer -30

clear @s *[minecraft:custom_data~{component:"twinscythes"}]
function pvp:c/twinscythes/get2
