execute if score @s deception.timer matches -1 positioned ~ ~1 ~ if entity @n[type=#pvp:projectile,distance=..3] as @n[type=#pvp:projectile,distance=..3] run function pvp:c/deception/tp/check
