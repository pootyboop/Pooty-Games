...
execute as @n[tag=cheatchecker] store result score @s dummy run data get entity @s Pos[0]
execute as @a[tag=trialing] store result score @s dummy run data get entity @s Pos[0]
execute as @n[tag=cheatchecker] if score @s dummy > @a[tag=trialing,limit=1] dummy run kill @a[tag=trialing,limit=1]
